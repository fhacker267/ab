...
<div class="hero-wrap js-fullheight" style="background-image: url('images/bg_1.jpg');">
    ...
    <img src="{{ asset('storage/' . $hotels->image_path) }}" alt="" style="max-width: 600px; ">
</div>

                        </div>  
          		</div>
          		<div class="col-md-12 hotel-single mt-4 mb-5 ftco-animate">
          			<span>Our Best hotels  &amp; and restruents</span>
                      <h2>{{ $hotels->name }}</h2>
                      <p>{{ $hotels->description }}</p>
                      <span class="price">
                        <del>AED250</del> AED{{ $hotels->price }}
                      </span>
	...
	<br>
	<br>
    <span  style =" color :red"id="updatedPrice">Total Price: AED{{ $hotels->price }}</span>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="contactModalLabel"><h2 style="margin-left: 10px;">{{ $hotels->name }}</h2></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ url('/submit-hotel-booking') }}" method="POST">
